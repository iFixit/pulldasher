...
function Avatar({ user }: { user: SignatureUser }) {
  return (
    <img
      height={25}
      width={25}
      src={`https://avatars.githubusercontent.com/u/${user.id}?s=25`}
    />
  );
}
...
export function getLeaders(
  pulls: Pull[],
  extractSigsFromPull: (pull: Pull) => Signature[]
) {
  const recent = toDateString(
    new Date(Date.now() - 86400 * 1000 * DAYS_TO_EXAMINE)
  );
  ...
  return Array.from(sigCounts.values()).sort(
    (a: SigCount, b: SigCount) => b.count - a.count
  );
}
