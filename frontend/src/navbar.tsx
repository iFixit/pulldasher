...
export function Navbar(props: NavBarProps) {
   ...
   useEffect(
      () => setPullFilter('external_block', showExtBlocked ? null : isNotExternallyBlocked),
      [showExtBlocked]
   );
   useEffect(() => setPullFilter('drafts', showDrafts ? null : isNotDraftOrMine), [showDrafts]);
   useEffect(
         ...
         >
            <HStack alignSelf="center" flexGrow={1} flexBasis={sideWidth} spacing="2">
               ...
               <Menu closeOnSelect={false}>
                  ...
                  <MenuList>
                     <MenuItemOption key="Cryo" onClick={toggleShowCryo} isChecked={showCryo}>
                        Cryogenic Storage
                     </MenuItemOption>
                     <MenuItemOption
                        key="External"
                     ...
                     <MenuItemOption key="Draft" onClick={toggleShowDrafts} isChecked={showDrafts}>
                        Drafts
                     </MenuItemOption>
                  </MenuList>
               </Menu>
...
function SearchInput() {
   ...
   const updateSearchFilter = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
         ...
         patterns.length
            ? (pull: Pull) => {
                 return patterns.every(
                    pattern => pull.title.match(pattern) || pull.repo.match(pattern)
                 );
              }
            : null
      );
