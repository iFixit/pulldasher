...
function StatusLink({ status }: { status: CommitStatus }) {
  ...
  return (
    ...
    >
      ...
      <chakra.span fontWeight="bold">{status.data.context}:</chakra.span>
      <chakra.span>{status.data.description}</chakra.span>
...
function iconForStatus(status: CommitStatus): IconDefinition {
  ...
  return state === StatusState.pending
    ? faClock
...
export const CommitStatuses = memo(function CommitStatuses({
  pull,
}: {
  pull: Pull;
}) {
    ...
    <Popover isLazy>
      ...
      <Portal>
        <PopoverContent>
          ...
          <PopoverBody>
            {statuses.map((status) => (
              <StatusLink key={status.data.context} status={status} />
            ))}
          </PopoverBody>
        </PopoverContent>
      </Portal>
