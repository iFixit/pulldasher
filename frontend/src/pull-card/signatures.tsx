...
function SignaturesFlag({
  ...
  const requiredSignatures = required
    ? allSignatures.slice(0, required)
    : allSignatures;
...
export const Signatures = memo(function Signatures(props: SignaturesProps) {
  const { pull, signatures } = props;
  const noSignatures =
    signatures.current.length === 0 && signatures.old.length === 0;
...
function SignatureList({ sigs, pull }: { sigs: Signature[]; pull: Pull }) {
  return (
    <Box>
      {sigs.map((sig) => (
        <SignatureListItem
          key={String(sig.data.created_at)}
          sig={sig}
          pull={pull}
        />
      ))}
...
function SignatureListItem({ sig, pull }: { sig: Signature; pull: Pull }) {
  ...
  return (
    <chakra.a __css={styles} target="_blank" href={pull.linkToSignature(sig)}>
...
function colorForSignature(sig: Signature): string {
  if (sig.data.active) {
    return getUser() === sig.data.user.login
      ? "var(--signature-valid-mine)"
      : "var(--signature-valid)";
  } else {
    return getUser() === sig.data.user.login
      ? "var(--signature-invalid-mine)"
