...
module.exports = {
  ...
  getPull: function (repo, number) {
    return logErrors(githubRest.pulls
      .get(params({ pull_number: number }, repo))
        .then((res) => res.data),
        "Getting pull %s", number);
  },
  ...
  getOpenPulls: function (repo) {
    return logErrors(github.paginate(
      githubRest.pulls.list,
      params({ state: "open" }, repo)
    ), "Getting open pulls in repo %s", repo);
  },
  ...
  getAllPulls: function (repo) {
    return logErrors(github.paginate(
      githubRest.pulls.list,
      params({ state: "all" }, repo)
    ), "Getting all pulls in repo %s", repo);
  },
  ...
  getIssue: function (repo, number) {
    ...
    return logErrors(githubRest.issues
      .get(searchParams)
      .then((res) => res.data)
      .then(addRepo(searchParams)),
    "Getting issue %s in repo %s", number, repo);
  },
  ...
  getOpenIssues: function (repo) {
    ...
    return logErrors(github
      .paginate(githubRest.issues.listForRepo, searchParams)
      .then(filterOutPulls)
      .then(addRepo(searchParams)),
    "Getting open issues in repo %s", repo);
  },
  ...
  getAllIssues: function (repo) {
    ...
    return logErrors(github
      ...
      .then(filterOutPulls)
      .then(addRepo(searchParams)),
    "Getting all issues in repo %s", repo);
  },
  ...
  parse: function (githubPull) {
    ...
    return Promise.all([
      ...
      let checks = commitChecks.map(function (commitCheck) {
        let state = utils.mapCheckToStatus(
          commitCheck.conclusion || commitCheck.status
        );
        let desc = state;
...
function getIssueEvents(repo, number) {
  ...
  ), "Getting events for issue %s:%s", repo, number);
}
...
function getIssueComments(repo, number) {
  return logErrors(github.paginate(
    ...
    params({ issue_number: number }, repo)
  ), "Getting comments for issue %s:%s", repo, number);
}
...
function getReviews(repo, number) {
  ...
  ), "Getting reviews for pull %s:%s", repo, number);
}
...
function getPullReviewComments(repo, number) {
  return logErrors(github.paginate(
    ...
    params({ pull_number: number }, repo)
  ), "Getting pull review comments for pull %s:%s", repo, number);
}

function getCommit(repo, sha) {
  return logErrors(githubRest.repos
    .getCommit(params({ ref: sha }, repo))
    .then((res) => res.data),
  "Getting commit for %s:%s", repo, sha);
}
...
function getCommitStatuses(repo, ref) {
  ...
  "Getting commit status for %s:%s", repo, ref);
}

function getCommitChecks(repo, ref) {
  return logErrors(githubRest.checks
    .listForRef(params({ ref }, repo))
    .then((res) => res.data.check_runs)
    .then((check_runs) => check_runs || []),
  "Getting commit checks for %s:%s", repo, ref);
}
