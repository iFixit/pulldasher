...
export function getBotLogin() {
   if (!botLoginPromise) {
      botLoginPromise = logErrors(
         ...
         // request as not-bot (self only via an actual self-request). But this
         // lookup failing at startup is also the strongest available signal
         // that the bot's GitHub token is invalid/expired: sockets stay
         // healthy and webhook-driven DB updates keep flowing regardless (they
         // don't need this token), so every subsequent live-API refresh
         // (claim/release review, synchronize reconciliation, edited-comment
         // refreshes) would otherwise fail with zero operator-visible signal.
         // Surface it loudly; still non-fatal so startup completes.
         console.error(
            'Could not determine bot login -- likely cause: the bot GitHub ' +
               'token (config.github.token) is invalid or expired. Bot-detection ' +
               'will degrade (self-claims may misclassify), and live-API ' +
               'refreshes (claim/release review, synchronize reconciliation, ' +
               'edited-comment refreshes) will silently fail until it is ' +
               'rotated. Underlying error: %s',
            (err && err.message) || err
         );
         return null;
      });
...
export default {
   ...
   parse: function (githubPull) {
      ...
      return Promise.all([
         ...
         // graft the fields a list item lacks (diff stats + mergeable, see
         // diffStats above). Omitting mergeable here re-writes it as NULL on
         // every bulk/list refresh, so derive()'s `mergeable === false` never
         // matches and a genuinely-conflicted PR reads as ready-to-merge.
         ...
         githubPull.mergeable = fullPull.mergeable;
