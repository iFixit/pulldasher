...
module.exports = {
   ...
   getIssue: function(number) {
      var searchParams = params({ number: number });
   ...
   getAllIssues: function() {
      var searchParams = params({ state: 'all' });
...
/**
 * Returns a function that uses the search parameters to add the "repository"
...
function addRepo(params) {
   function addRepositoryField(ghIssue) {
      ghIssue.repository = {full_name: params.user + "/" + params.repo};
   }
