...
module.exports = {
  ...
  parse: function (githubPull) {
    ...
    var workflowRuns = getWorkflowRuns(repo, githubPull.head.sha);
    ...
    return Promise.all([
      ...
      let checks = workflowRuns.map(function (workflowRun) {
        let state = utils.mapCheckToStatus(
          workflowRun.conclusion || workflowRun.status
        );
        let desc = state;
        let url = workflowRun.html_url;
        let context = workflowRun.name;
...
function getWorkflowRuns(repo, ref) {
  return logErrors(
    github
      .paginate(
