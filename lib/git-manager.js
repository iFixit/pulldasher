...
module.exports = {
   ...
   getIssue: function(repo, number) {
      ...
      return github.paginate(githubRest.issues.get, searchParams).then(addRepo(searchParams));
   },
   ...
   getOpenIssues: function(repo) {
      ...
      return github.paginate(githubRest.issues.listForRepo, searchParams).then(filterOutPulls).then(addRepo(searchParams));
   },
   ...
   getAllIssues: function(repo) {
      ...
      return github(githubRest.issues.listForRepo, searchParams).then(filterOutPulls).then(addRepo(searchParams));
   },
...
function getIssueEvents(repo, number) {
   ...
   return github.paginate(githubRest.issues.listEventsForRepo, params({ issue_number: number }, repo))
}
...
function getPullReviewComments(repo, number) {
   ...
   return github.paginate(githubRest.pullRequests.getComments,params({ issue_number: number }, repo))
}

function getCommit(repo, sha) {
   return rateLimit(githubRest.repos.getCommit)(params({ sha }, repo))
                      .then(res => res.data);
}

function getCommitStatuses(repo, ref) {
   debug("Getting commit status for %s", ref);
   return rateLimit(githubRest.repos.getCombinedStatusForRef)(params({ ref }, repo))
