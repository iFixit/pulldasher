...
api.getCommitStatuses      = denodeify(github.statuses.get);
   ...
   parse: function(githubPull) {
      ...
      return Promise.all([reviewComments,
       ...
       .then(function(results) {
         ...
         var statuses = commitStatuses.map(function(commitStatus) {
            return new Status({
               repo:          repo,
         ...
         var pull = Pull.fromGithubApi(githubPull, signatures, comments, statuses, labels);
...
function getCommitStatuses(repo, sha) {
   return new Promise(function(resolve, reject) {
      ...
      api.getCommitStatuses(params({
         sha: sha
      }, repo)).then(function(statuses) {
         resolve(statuses);
      });
   });
}
