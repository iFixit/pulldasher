...
/**
 * Resolve the GitHub App private key from one of three sources
 * (in order of priority):
 *   1. GITHUB_APP_PRIVATE_KEY environment variable
 *   2. A file path in config.github.privateKey
 *   3. An inline PEM string in config.github.privateKey
 *
 * Returns the PEM string or null if no private key is configured.
 */
function resolvePrivateKey() {
  // Environment variable takes highest priority
  if (process.env.GITHUB_APP_PRIVATE_KEY) {
    gitDebug("Using GitHub App private key from GITHUB_APP_PRIVATE_KEY env var");
    return process.env.GITHUB_APP_PRIVATE_KEY;
  }

  const configKey = config.github.privateKey;
  if (!configKey) {
    return null;
  }

  // If it looks like a PEM key, use it directly
  if (configKey.includes("-----BEGIN")) {
    gitDebug("Using inline GitHub App private key from config");
    return configKey;
  }

  // Otherwise treat it as a file path
  if (existsSync(configKey)) {
    gitDebug("Reading GitHub App private key from file: %s", configKey);
    return readFileSync(configKey, "utf8");
  }

  throw new Error(
    `GitHub App private key file not found: ${configKey}. ` +
      "Provide the PEM string directly, a valid file path, or set the " +
      "GITHUB_APP_PRIVATE_KEY environment variable."
  );
}

/**
 * Build the Octokit auth configuration.
 *
 * If GitHub App credentials (appId, privateKey, installationId) are present,
 * uses auto-rotating installation tokens via @octokit/auth-app.
 * Otherwise falls back to a static token (classic PAT) with a deprecation
 * warning.
 */
function buildAuthConfig() {
  if (config.github.appId) {
    ...
    if (!privateKey) {
      throw new Error(
        "GitHub App appId is configured but no private key was found. " +
          "Set config.github.privateKey or the GITHUB_APP_PRIVATE_KEY env var."
      );
    }
    if (!config.github.installationId) {
      throw new Error(
        "GitHub App appId is configured but installationId is missing. " +
          "Set config.github.installationId to the numeric installation ID."
      );
    }

    console.log(
      "Using GitHub App authentication (appId: %s, installationId: %s)",
      config.github.appId,
  ...
  if (config.github.token) {
    console.warn(
      "WARNING: Using a classic Personal Access Token for GitHub API auth. " +
        "Classic PATs are deprecated. Migrate to a GitHub App by setting " +
        "appId, privateKey, and installationId in your config. " +
        "See config.example.js for details."
    );
    return {
  ...
  throw new Error(
    "No GitHub authentication configured. Set either GitHub App credentials " +
      "(appId, privateKey, installationId) or a classic token in your config."
  );
}
