...
// Resolves (204) when `user.username` is in the configured org/team, rejects
// (404) otherwise -- the same check the web login runs. Exported so the
// /api/v1 Bearer auth gates on identical org membership.
export function confirmOrgMembership(user) {
...
export default {
   ...
   setupRoutes: function (app) {
      ...
      app.get('/', auth);
      // the legacy v1 board runs at /v1 — gate its page the same as the root so
      // an unauthenticated hit redirects to GitHub login instead of loading a
      // board that can't fetch its token
      app.get('/v1', auth);
      app.get('/v1/', auth);
      app.get('/token', auth);
      // Same auth gate as /token: register the middleware here, ahead of the
      // route's real handler in app.js (Express runs same-path GET handlers in
      // registration order), so the query never runs for an unauthenticated request.
      app.get('/stats-history', auth);
      app.get('/user-names', auth);
   },
};
