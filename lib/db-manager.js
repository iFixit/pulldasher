...
module.exports = {
   ...
   /**
    * Inserts a commit status into the DB. Takes a Status object as an an
    * argument.
    */
   ...
   getPull: function(number) {
      ...
      return new Promise(function(resolve, reject) {
         db.query(q_select, number, function(err, rows) {
            ...
            commitStatus = self.getCommitStatus(pullData.head.sha);

            Promise.all([signatures, commitStatus]).then(function(resolved) {
               signatures = resolved[0];
               commitStatus = resolved[1];
               resolve(new Pull(pullData, signatures, commitStatus));
            });
         });
      });
   },
