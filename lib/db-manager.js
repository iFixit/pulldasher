...
module.exports = {
   ...
   getPull: function(number) {
      ...
      return new Promise(function(resolve, reject) {
         db.callback_query(q_select, number, function(err, rows) {
            ...
            Promise.all([signatures, comments, commitStatus, labels])
   ...
   getPullNumberFromSha: function(sha) {
      ...
      return new Promise(function(resolve, reject) {
         db.callback_query(q_select, sha, function(err, rows) {
            if (err) { return reject(err); }
   ...
   getSignatures: function(number, repo) {
      ...
      return db.query(q_select, [number, repo]).then(function(rows) {
         return rows.map(function(row) {
            return new Signature(Signature.getFromDB(row));
         });
      });
   },
   ...
   getComments: function(number, repo) {
      ...
      return db.query(q_select, [number, repo]).then(function(rows) {
         return rows.map(function(row) {
            return new Comment(Comment.getFromDB(row));
         });
      });
   },
   ...
   getCommitStatus: function(sha) {
      ...
      return db.query(q_select, sha).then(function(rows) {
         if (rows.length === 1) {
            return new Status(Status.getFromDB(rows[0]));
         } else {
            return null;
         }
      });
      ...
      return new Promise(function(resolve, reject) {
         ...
         db.query(q_select, [number, repo], function(err, rows) {
            ...
            var labels = rows.map(function(row) {
               return new Label(Label.getFromDB(row), number);
            });
   ...
   deleteLabels: function deleteLabels(number) {
      ...
      return new Promise(function(resolve, reject) {
         ...
         db.query(q_delete, [number, config.repo.name], function(err, rows) {
            if (err) { return reject(err); }

            resolve();
            debug('deleteLabels: deleting from pull #%s', number);
         });
      });
   },
