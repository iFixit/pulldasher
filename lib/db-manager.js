...
module.exports = {
   ...
   getAllSignatures: function(number, repo) {
      var q_select = '\
         SELECT pull_signatures.*, comments.*, CONCAT(first_name, " ", substring(last_name, 1, 1)) AS realname FROM comments \
         JOIN pull_signatures USING (comment_id, number) \
         JOIN ***REMOVED*** ON (pull_signatures.user = github_username) \
         WHERE number = ? AND repo_name = ? \
